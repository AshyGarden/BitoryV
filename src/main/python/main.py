...
def get_crypto_price(ticker="BTC"):                   # 가상화폐의 가격을 가져오는 사용자 함수
    ...
    mse = math.sqrt(mean_squared_error(df['y'], forecast['yhat'][:4320]))
    mae = math.sqrt(mean_absolute_error(df['y'], forecast['yhat'][:4320]))
    mape = math.sqrt(mean_absolute_percentage_error(df['y'], forecast['yhat'][:4320]))

    output_mse = f"평균 제곱 오차 (MSE) : {mse}"
    output_mae = f"평균 절대 오차 (MAE) : {mae}"
    output_mape = f"평균 절대 비율 오차 (MAPE) : {mape}"
...
# 서비스 계정 키 파일 경로
# 현재 작업 디렉토리에 있는 JSON 키 파일 경로를 환경 변수로 설정
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'mystic-sun-405907-192278ca6d95.json'

# 서비스 계정 키 파일로부터 인증 정보 생성
...
async def process_logs(log: TextLog):
   ...
   # 변환된 음성을 파일로 저장
    ...
    with open('../resources/static/assets/images/output.mp3', "wb") as out:
        out.write(response.audio_content)
        print(f"음성 파일이 저장되었습니다.")
    try:
